...
.warn-box{background:#fff4e5;border-right:4px solid #e8a93a;border-radius:8px;
  padding:10px 13px;font-size:.83rem;color:#7a5210;margin-bottom:8px;line-height:1.9}
</style>"""
...
def calculate_line_length(coords):
    """حساب طول الخط من الإحداثيات (بالمتر)"""
    if len(coords) < 2:
        return 0.0
    total_length = 0.0
...
def calculate_pipe_details(pipe_length, diameter_mm, avg_depth, num_traps=None, num_manholes=None):
    """حساب تفاصيل الأنابيب. يرجع (result, warnings)"""
    warnings = []
    try:
...
st.markdown('<div class="hdr"><h1>🌊 حاسبة شبكات تصريف السيول مع الخرائط التفاعلية</h1><div class="bdg">V3.1</div></div>', unsafe_allow_html=True)
...
with tab1:
    ...
    if not FOLIUM_AVAILABLE:
        st.markdown(
            ...
            'تأكد من وجود ملف requirements.txt يحتوي عليها، ثم أعد تشغيل التطبيق. '
            'بإمكانك إدخال طول الأنبوب يدوياً في التبويبين الآخرين دون الحاجة للخريطة.</div>',
        ...
        with col2:
            ...
            if st.session_state['drawn_line_length'] > 0:
                ...
                if st.button("✅ استخدام هذا الطول في التبويبين التاليين", key="use_line_length", use_container_width=True):
...
with tab3:
    ...
    st.markdown('<div class="map-info">💡 يمكنك استخدام الطول المرسوم على الخريطة من التبويب الأول (زر "استخدام هذا الطول") أو تعديله يدوياً هنا.</div>', unsafe_allow_html=True)
