...
def do_login(n, user, pwd, auth):
    ...
    # بسيط: اقرأ من ملف secrets.toml إذا موجود، وإلا قبل أي بيانات للتجربة
    try:
        import toml
...
# ══ اختيار الخط من الخريطة (Dash-Leaflet Polyline click) ══
@app.callback(
...
def on_line_click(n_clicks_list, sel, feats, meta):
    ...
    # تهيئة meta للخط الجديد إذا لم يكن موجوداً
    meta = dict(meta or {})
